"""Extended-window backtest + per-sub-model & ensemble metrics, parameterized.

Variant of analyze_extended_windows.py that takes --cache-tag so it can run
against the 1y/2y extended-training caches without clobbering the 2019-12
results.

Run:
    uv run python analyze_extended_windows_v2.py --cache-tag train1y
    uv run python analyze_extended_windows_v2.py --cache-tag train2y
    uv run python analyze_extended_windows_v2.py --cache-tag base   # original cache
"""
...
WINDOWS_DAYS = (365, 730, 1095, 1460)  # 1y, 2y, 3y, 4y
TRADING_DAYS = 365
STAKE_USD = 100.0
...
def _sharpe_psr(rets: np.ndarray) -> dict:
    ...
    if n < 5 or rets.std(ddof=1) == 0:
        return dict(sharpe=float("nan"), psr=float("nan"), sortino=float("nan"),
                    sharpe_per_period=float("nan"), sr_var=float("nan"))
...
def _single_cell_metrics(c: CellSpec, window_days: int,
    ...
    res = run_backtest(p, cl, stake_usd=STAKE_USD,
                       up_threshold=c.prob_TH, down_threshold=0.50)
    eq = res.equity_curve
    bh = res.buy_and_hold_curve.reindex(eq.index)
    if eq.empty:
        return {}

    total_return = float(eq.iloc[-1]) / STAKE_USD - 1
    ...
    pf = _profit_factor([t.pnl_usd for t in res.trades])
    qb = _quarterly_outperform(eq, bh)
    bear_pp = _bear_outperform_pp(eq, bh)
    n_wins = sum(1 for t in res.trades if t.pnl_usd > 0)
    win_rate = n_wins / res.n_trades if res.n_trades else 0.0
    return dict(
        model=_cell_key(c),
...
def _split_capital_metrics(model_name: str, picks: list[CellSpec],
                           window_days: int,
                           preds_by_cell: dict[str, pd.DataFrame],
                           close: pd.Series) -> dict:
    N = len(picks)
...
def main():
    ...
    ap.add_argument("--cache-tag", required=True,
                    help="'base' for preds_cache_ebm_ebm_extended/, otherwise "
                         "preds_cache_ebm_ebm_extended_{tag}/")
    args = ap.parse_args()
    ...
    print("=" * 78)
    print(f"Extended-window backtest v2 — windows: {WINDOWS_DAYS}d")
    ...
    for c in CELLS:
        ...
        if not path.exists():
            ...
            continue
        preds_by_cell[_cell_key(c)] = pd.read_parquet(path)

    bar = build_utc2130_daily(DATA_DIR / "btc_15m.parquet")
    close = bar.set_index("close_time")["close"]

    summary_rows = []
    # 5 sub-models individually
    for c in CELLS:
        if _cell_key(c) not in preds_by_cell:
            continue
        for w in WINDOWS_DAYS:
            m = _single_cell_metrics(c, w, preds_by_cell, close)
            if m:
                summary_rows.append(m)
    # 4 ensembles
    for model_name, cell_keys in ENSEMBLES.items():
        picks = [by_key[k] for k in cell_keys]
        if any(_cell_key(p) not in preds_by_cell for p in picks):
            continue
        for w in WINDOWS_DAYS:
            m = _split_capital_metrics(model_name, picks, w, preds_by_cell, close)
            if m:
                summary_rows.append(m)
    ...
    out_csv = out_dir / "topN_ensemble_summary.csv"
