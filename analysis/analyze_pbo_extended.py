"""PBO via CSCV at extended windows (1095d / 1460d).

Mirrors analyze_pbo.py methodology but reads from data/preds_cache_ebm_ebm_extended/
and runs for windows 1095 and 1460. Strategy universe per cell:
  22 labels (11 static th* + 11 dynamic dynk*) × 21 prob_TH = 462 strategies.
CSCV with S=16 contiguous return blocks → C(16,8)=12,870 IS/OOS combos.

Output: reports/utc2130_extended/pbo_summary_extended.csv

Run:
    uv run python analyze_pbo_extended.py
"""
...
def precompute_cell(cell: str, close: pd.Series, window_days: int
                    ) -> tuple[np.ndarray, ...]:
...
def main():
    ...
    for window in WINDOWS:
        ...
        for cell in CELLS:
            ...
            bs, bq, bn = precompute_cell(cell, close, window)
            if len(bs) == 0:
    ...
    out_csv = OUT_DIR / "pbo_summary_extended.csv"
